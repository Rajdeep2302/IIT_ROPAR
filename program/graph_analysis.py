...
def skeletonise_image(bw: np.ndarray) -> np.ndarray:
    # Apply skeletonization with a pre-smoothing step to preserve connectivity
    blurred_bw = cv.GaussianBlur(bw.astype(np.float32), (3, 3), 0.5)
    skel = skeletonize(blurred_bw > 0).astype(np.uint8)
    
    # Minimal morphological thinning to avoid breaking edges
    kernel = np.ones((3, 3), np.uint8)
    skel = cv.morphologyEx(skel, cv.MORPH_CLOSE, kernel, iterations=1)  # Close small gaps
    return skel
...
def find_nodes(skel: np.ndarray) -> List[Tuple[int, int]]:
    ...
    return [(int(round(r)), int(round(c))) for r, c in centroids if not np.isnan(r) and not np.isnan(c)]

def euclidean_distance(p: Tuple[int, int], q: Tuple[int, int]) -> float:
    return float(np.hypot(p[0] - q[0], p[1] - q[1]))

def find_connections(skel: np.ndarray, nodes: List[Tuple[int, int]], max_distance: int = 150) -> Dict[int, List[int]]:
